...
func duck_buses():
	for bus_name in ["SFX","BGM"]:
		...
		bus_levels[bus_idx] = AudioServer.get_bus_volume_db(bus_idx)
		AudioServer.set_bus_volume_db(bus_idx, bus_levels[bus_idx] - 6.0)
	
func restore_buses():
	for bus_name in ["SFX","BGM"]:
		var bus_idx = AudioServer.get_bus_index(bus_name)
		AudioServer.set_bus_volume_db(bus_idx, bus_levels[bus_idx])

## Creates a sound effect if the limit has not been reached. Pass [param type] for the SoundEffect to be queued.
func create_priority_audio(type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		var sound_effect: SoundEffect = sound_effect_dict[type]
		if sound_effect.has_open_limit():
			sound_effect.change_audio_count(1)
			...
			new_audio.bus = "PRIO"
			new_audio.finished.connect(restore_buses)
